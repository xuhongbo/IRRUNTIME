    ...
    <>
      {presets && presets.length > 0 && (
        <FormControl fullWidth size="small" sx={{ mt: 2 }}>
            <InputLabel id="preset-label">预设</InputLabel>
            <Select
  ...
  const renderOutputs = () => (
      <>
        {outputs && Object.keys(outputs).length > 0 && (
          <Box sx={{ mt: 2 }}>
             <Divider sx={{ my: 1 }} />
            <Typography variant="caption" fontWeight="bold" color="text.secondary">输出</Typography>
    ...
    return (
      <Box sx={{ height: "100%", overflow: "auto", p: 2 }} data-testid="runner-root">
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="subtitle1" fontWeight={700}>运行面板</Typography>
            <Chip 
                label={statusLabel} 
                size="small" 
    ...
    <Box sx={{ height: "100%", overflow: "auto", p: 2 }} data-testid="runner-root">
      <Card variant="outlined" sx={{ mb: 2 }}>
          <CardContent sx={{ pb: 1 }}>
              ...
              {viewModel.kind === "waiting" && <Alert severity="info" sx={{ mt: 2 }}>正在等待异步流程...</Alert>}
              {viewModel.kind === "error" && <Alert severity="error" sx={{ mt: 2 }}>执行已终止。</Alert>}
              {viewModel.kind === "done" && <Alert severity="success" sx={{ mt: 2 }}>流程已完成。</Alert>}
          </CardContent>
